...
const getRegisteredSalt = (email: string) => {
  // This call will check to see if email is already registered, and return salt if so, or generate, store, and return salt if not...
...
const regenerateReadwriteKeypair = async (
  ...
  try {
    ...
    if (registeredSalt) {
      const { hash } = await pwHash(password, registeredSalt.slice(0, 16))
      // WARNING: Currently there is intentionally no check for the correct hash, which results in creating the A NEW AGENT keypair/ID rather than recreating the the current Agent's correct keypair/ID.
      // >> TODO: Find a smooth way to handle this or provide feedback about this for the user (ie. check if user/agentId is an exisiting user/agentId, and if not send message saying it looks like they craeted a new user due to an incorrect password provision).
      const keypair = await Keypair.newFromSeed(hash)
      return keypair
    }
  } catch (e) {
    return console.error('No salt returned from saltmine. Unable to log in.', e)
  }
}
